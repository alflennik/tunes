...
const siteBuildScript = async ({ environment }) => {
  ...
  const checkGit = async () => {
    ...
    const checkBranches = async () => {
      ...
      if (branches.match(`\\s${environment}\\n`) === null) {
        throw new Error(`Your ${environment} branch is missing`)
      }
    }
  ...
  const clearBuildFolder = async () => {
    const fileNames = await fs.readdir(buildFolder)
    await Promise.all(
      fileNames.map(async (fileName) => {
        const filePath = path.resolve(buildFolder, fileName)
  ...
  const copyAssets = async () => {
    await Promise.all(
      assetLocations.map(async (assetLocation) => {
        ...
        const assetDestination = path.resolve(buildFolder, assetLocation)

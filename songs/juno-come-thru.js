export default {
  descriptions: [
    {
      time: 0,
      text: "A finger depresses buttons on an old CD-player.",
    },
    {
      time: 6.692,
      text: "Shoes tap to the beat.",
    },
    {
      time: 25.204,
      text: "Two people, a blondie girl and a bassist dude, loom over the camera, the wide angle lens making them appear Godzilla-size. They seem to be in a park somewhere in suburban Los Angeles.",
    },
    {
      time: 43.633,
      text: "They bob and bounce like surfers riding on the massive beat.",
    },
    {
      time: 67.047,
      text: "They also appear in a basement-converted-makeshift studio, standing on a backdrop colored a startling shade of pink.",
    },
    {
      time: 101.045,
      text: "The camera shakes slightly, probably because the cameraman can't help but dance along.",
    },
    {
      time: 110.868,
      text: "She spells out the numbers with her fingers to make sure you receive the positional message: the 405 highway in Los Angeles.",
    },
    {
      time: 126.641,
      text: "They take turns approaching the fisheye lens, letting it warp and expand their shapes to heroic proportions.",
    },
    {
      time: 160.646,
      text: "Her whole body bounces as she releases that confession.",
    },
  ],
  analysis: {
    text: "Analysis: The massive groove they produce in this song is what you'll experience if you take the plunge and move to Los Angeles, like you've been dreaming.",
  },
}
